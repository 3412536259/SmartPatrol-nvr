...
int main() {
    ...
    DownloadVideoFile info("2","nvr_001",result.fileList[0].filename,result.fileList[0].starttime,result.fileList[0].endtime, std::to_string(result.fileList[0].filesize));
    DownloadFile out;

    // 1️⃣ 启动下载（只启动，不阻塞）
    bool ok = hkNVR->downloadRecordFile(info, out);
    if (!ok) {
        std::cerr << "[错误] 启动下载失败" << std::endl;
    } else {
    ...
    std::cout << "out.isFinished() + " << out.isFinished() << std::endl;    
    ...
    // 3️⃣ 下载完成 
    if (out.isSuccess()) {
        std::cout << "[成功] 下载完成！文件路径: "
                << out.localPath << std::endl;
    } else {
        std::cerr << "[失败] 下载失败，错误原因: "
                << out.processError << std::endl;
    }
